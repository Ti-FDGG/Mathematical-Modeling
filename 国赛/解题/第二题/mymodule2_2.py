...
def calculate_stability(past_profits):
    # 对往年利润进行一阶差分
    profit_diffs = np.diff(past_profits)
    # 计算标准差
    stability = np.std(profit_diffs)

    return stability

def calculate_score(profits, stability, weights):
    score = profits * weights[0] - stability * weights[1]
    return score

...
def add_constraints(prob, variables, binary_variables, actual_sales, excess_yield, 
                    ...
                    min_area_percent, max_plots, correlation_matrix, regression_params_dict):
    '''
    expected_sales_data: \n
    ...
    #         if i != j:
    #             prob += actual_sales[crop1.crop_name] * correlation_matrix[i, j] <= actual_sales[crop2.crop_name]
    ...
    #     # 根据作物类型选择对应的回归参数
    #     params = regression_params_dict.get(crop.crop_type)
    #     if params:
    #         prob += actual_sales[crop.crop_name] == params['intercept'] + params['price_coef'] * price + params['cost_coef'] * cost
...
def optimize_planting_strategy(new_fields, crops, expected_sales_data, df_last1_result, 
                               df_last2_result, params_list, correlation_matrix, regression_params_dict):
    """
    优化种植策略
    ...
    expected_sales_data (DataFrame): 作物预期销售量数据
    ...
    prob = add_constraints(prob, variables, binary_variables, actual_sales, 
                           excess_yield, crops, new_fields, expected_sales_data, df_last1_result, df_last2_result,
                           min_area_percent, max_plots, correlation_matrix, regression_params_dict)

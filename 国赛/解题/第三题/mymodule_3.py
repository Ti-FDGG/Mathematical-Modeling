...
def get_merged_df(df21, df22):
    result1 = df21.groupby(['作物编号', '地块类型', '种植季次'])['种植面积/亩'].sum().reset_index()
    result1.columns = ['作物编号', '地块类型', '种植季次', '种植面积/亩']
    
    merged_df = pd.merge(result1, df22[['作物编号', '作物名称', '作物类型', '地块类型', '种植季次', '亩产量/斤', '种植成本/(元/亩)', '销售单价/(元/斤)']], 
                         on=['作物编号', '种植季次', '地块类型'], how='left')
    ...
    for crop in crops:
        for field in crop.planting_fields:
            ...
            if crop_yield is not None and crop_cost is not None:
                field.append(crop_yield) # 第三项是亩产量
                field.append(crop_cost) # 第四项是种植成本
    ...
    for v in variables.values():
        ...
        for field in new_fields:
            if field.field_name == field_name and field.season == season:
                ...
                break
def update_template(df_template, new_fields):
    # 将读取到的表格中的NaN取值为0
